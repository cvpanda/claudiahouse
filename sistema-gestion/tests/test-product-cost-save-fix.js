...
const purchaseData = {
  ...
  exchangeRate: 1330.0,
  freightCost: 2470.0, // Ya en ARS (el usuario mencionó $ 2.470,00)
  customsCost: 0,
  taxCost: 661000.0, // ARS
  insuranceCost: 0,
  otherCosts: 0,
...
const userSubtotalARS = 1490863.5;
const thisItemSubtotal = 20 * 1263.5; // $25.270
...
function calculateCorrectProductCost(purchase, item) {
  ...
  const finalUnitCost =
    (item.unitPricePesos || 0) + distributedCosts / (item.quantity || 1);
...
console.log("\n📊 Datos de la compra:");
console.log(
  `Tipo de cambio: ${purchaseData.exchangeRate.toLocaleString("es-AR")}`
);
console.log(
  `Flete (USD): ${(
    purchaseData.freightCost * purchaseData.exchangeRate
  ).toLocaleString("es-AR", { minimumFractionDigits: 2 })}`
);
console.log(
  `Impuestos (ARS): $${purchaseData.taxCost.toLocaleString("es-AR", {
...
console.log(
  `Precio unitario (USD): ${item.unitPriceForeign.toLocaleString("es-AR", {
    minimumFractionDigits: 2,
  })}`
);
console.log(
  `Precio unitario (ARS): $${item.unitPricePesos.toLocaleString("es-AR", {
    minimumFractionDigits: 2,
  })}`
);
...
console.log(
  `Subtotal total compra: $${calculation.subtotalPesos.toLocaleString("es-AR", {
    minimumFractionDigits: 2,
  })}`
);
console.log(
  `Total costos importación: $${calculation.totalImportCosts.toLocaleString(
    "es-AR",
    { minimumFractionDigits: 2 }
  )}`
);
console.log(
  `Subtotal del item: $${calculation.itemSubtotalPesos.toLocaleString("es-AR", {
    minimumFractionDigits: 2,
  })}`
);
console.log(
  `Costos distribuidos (total): $${calculation.distributedCosts.toLocaleString(
    "es-AR",
    { minimumFractionDigits: 2 }
  )}`
);
console.log(
  `Costos distribuidos (por unidad): $${calculation.distributedCostsPerUnit.toLocaleString(
    "es-AR",
    { minimumFractionDigits: 2 }
  )}`
);
...
console.log("\n✅ Resultado final:");
console.log(
  `❌ Costo incorrecto (solo unitario): $${item.unitPricePesos.toLocaleString(
    "es-AR",
    { minimumFractionDigits: 2 }
  )}`
);
console.log(
  `✅ Costo correcto (con distribución): $${calculation.finalUnitCost.toLocaleString(
    "es-AR",
    { minimumFractionDigits: 2 }
  )}`
);
...
const expectedFinalCost = 4607.8;
const calculatedFinalCost = calculation.finalUnitCost;

console.log("\n🎯 Verificación con datos del usuario:");
console.log(
  `Costo Final Unit. esperado: $${expectedFinalCost.toLocaleString("es-AR", {
    minimumFractionDigits: 2,
  })}`
);
console.log(
  `Costo Final Unit. calculado: $${calculatedFinalCost.toLocaleString("es-AR", {
    minimumFractionDigits: 2,
  })}`
);
console.log(
  `¿Son iguales? ${
    Math.abs(calculatedFinalCost - expectedFinalCost) < 0.01 ? "✅ SÍ" : "❌ NO"
  }`
);

const incorrectCost = 1825.79;
console.log(
  `Costo incorrecto guardado: $${incorrectCost.toLocaleString("es-AR", {
    minimumFractionDigits: 2,
  })}`
);
console.log(
  `Diferencia con el correcto: $${(
    calculatedFinalCost - incorrectCost
  ).toLocaleString("es-AR", { minimumFractionDigits: 2 })}`
);
...
if (Math.abs(calculatedFinalCost - expectedFinalCost) < 0.01) {
  ...
  console.log(
    `✅ El producto tendrá costo $${calculatedFinalCost.toLocaleString(
      "es-AR",
      { minimumFractionDigits: 2 }
    )} en lugar de $${incorrectCost.toLocaleString("es-AR", {
      minimumFractionDigits: 2,
    })}`
  );
} else {
  ...
  console.log(
    `❌ Esperado: ${expectedFinalCost}, Calculado: ${calculatedFinalCost}`
  );
}
