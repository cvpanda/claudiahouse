...
async function testComboModalFunctionality() {
  ...
  try {
    ...
    console.log(
      `✅ Test multi-selección: ${testSelections.length} productos seleccionados`
    );
    ...
    console.log(
      `✅ Resultados de búsqueda "test": ${searchResults.length} productos`
    );
    ...
    if (page1.length > 0) {
      ...
      console.log(
        `   ID: ${sampleProduct.id} (tipo: ${typeof sampleProduct.id})`
      );
      ...
      console.log(
        `   Categoría: ${sampleProduct.category?.name || "Sin categoría"}`
      );
    }
...
// Ejecutar test
testComboModalFunctionality().catch(console.error);
