...
async function debugPurchase() {
  try {
    ...
    purchase.items.forEach((item, index) => {
      ...
      console.log(
        `   Subtotal ARS: ${
          item.subtotalPesos || item.quantity * item.unitPricePesos
        }`
      );
    ...
    const insuranceCostARS =
      (purchase.insuranceCost || 0) * (purchase.exchangeRate || 1);
    ...
    console.log(
      `Subtotal total productos: $${totalSubtotalPesos.toLocaleString("es-AR", {
        minimumFractionDigits: 2,
      })}`
    );
    console.log(
      `Total costos importación: $${totalImportCosts.toLocaleString("es-AR", {
        minimumFractionDigits: 2,
      })}`
    );
    ...
    purchase.items.forEach((item, index) => {
      ...
      console.log(`\n📊 ${item.product.name}:`);
      console.log(
        `   Subtotal item: $${itemSubtotalPesos.toLocaleString("es-AR", {
          minimumFractionDigits: 2,
        })}`
      );
      console.log(
        `   Costos distribuidos totales: $${proportionalCosts.toLocaleString(
          "es-AR",
          { minimumFractionDigits: 2 }
        )}`
      );
      console.log(
        `   Costo distribuido por unidad: $${costPerUnit.toLocaleString(
          "es-AR",
          { minimumFractionDigits: 2 }
        )}`
      );
      console.log(
        `   COSTO FINAL UNITARIO: $${finalUnitCost.toLocaleString("es-AR", {
          minimumFractionDigits: 2,
        })}`
      );
      ...
      console.log(
        `   ¿Es correcto? ${
          Math.abs(item.product.cost - finalUnitCost) < 0.01 ? "✅ SÍ" : "❌ NO"
        }`
      );
    });
  } catch (error) {
