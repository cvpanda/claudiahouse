...
async function testSalesImprovements() {
  try {
    ...
    console.log(`📦 Productos encontrados con imagen: ${productsWithImages.length}`);
    productsWithImages.forEach((product, index) => {
      console.log(`  ${index + 1}. ${product.name} (Stock: ${product.stock})`);
      console.log(`     - SKU: ${product.sku || 'Sin SKU'}`);
      console.log(`     - Mayorista: $${product.wholesalePrice} | Minorista: $${product.retailPrice}`);
      console.log(`     - Imagen: ✅ Disponible`);
      console.log(`     - Stock: ${product.stock === 0 ? '❌ SIN STOCK' : '✅ Con stock'}`);
      console.log('');
    });
    ...
    console.log(`📉 Productos sin stock encontrados: ${productsWithoutStock.length}`);
    ...
    console.log('   📸 Imágenes más grandes (w-20 h-20 en modal, w-16 h-16 en lista)');
    ...
    console.log('   🚫 Productos sin stock permitidos (no se resta del stock si es 0)');
    console.log('   ⚡ Botón "Agregar individual" para casos rápidos');
    console.log('   🎨 Estados visuales claros (seleccionado, agregado, sin stock)');
    ...
    console.log(`   ✅ AHORA: Todos los ${totalProducts} productos disponibles`);
