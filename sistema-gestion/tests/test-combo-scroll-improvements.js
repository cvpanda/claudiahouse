...
async function testComboScrollImprovements() {
  ...
  try {
    console.log('🧪 Probando mejoras de scroll infinito para modal de combos...\n');
    ...
    console.log(`📄 Primera página cargada: ${firstPage.length} productos (límite: ${firstPageLimit})`);
    
    // 3. Calcular páginas totales con nuevo límite
    const totalPages = Math.ceil(totalProducts / firstPageLimit);
    console.log(`📋 Páginas totales con límite de ${firstPageLimit}: ${totalPages}`);
    ...
    if (totalPages > 1) {
      ...
      console.log(`📄 Segunda página (scroll infinito): ${secondPage.length} productos`);
      console.log(`📊 Total acumulado: ${firstPage.length + secondPage.length} productos`);
    }
    ...
    console.log(`🔍 Búsqueda "${searchTerm}": ${searchResults.length} resultados en primera página`);
    ...
    const stockCategories = {
      sinStock: stockAnalysis.filter(s => s.stock === 0).reduce((acc, s) => acc + s._count.stock, 0),
      pocoStock: stockAnalysis.filter(s => s.stock > 0 && s.stock <= 5).reduce((acc, s) => acc + s._count.stock, 0),
      stockNormal: stockAnalysis.filter(s => s.stock > 5).reduce((acc, s) => acc + s._count.stock, 0)
    };
    
    console.log('\n📦 Análisis de stock:');
    console.log(`   • Sin stock: ${stockCategories.sinStock} productos`);
    console.log(`   • Poco stock (1-5): ${stockCategories.pocoStock} productos`);
    console.log(`   • Stock normal (6+): ${stockCategories.stockNormal} productos`);
