...
const purchaseData = {
  ...
  // Costos en USD
  freightCost: 2470.0 / 1330, // USD (el usuario ingresó $2.470 ARS, lo convertimos a USD)
  insuranceCost: 0,
  otherCosts: 0,
...
console.log(
  ...
  }): ${purchaseData.subtotalForeign.toLocaleString("es-AR", {
    minimumFractionDigits: 2,
  })}`
);
console.log(
  `Subtotal (ARS): $${purchaseData.subtotalPesos.toLocaleString("es-AR", {
    minimumFractionDigits: 2,
  })}`
);
...
console.log("\n🔧 Cálculo corregido:");
console.log(
  `Flete convertido a ARS: $${result.freightCostARS.toLocaleString("es-AR", {
    minimumFractionDigits: 2,
  })}`
);
console.log(
  `Impuestos (ARS): $${result.taxCostARS.toLocaleString("es-AR", {
    minimumFractionDigits: 2,
  })}`
);
console.log(
  `Total costos adicionales: $${result.totalImportCosts.toLocaleString(
    "es-AR",
    { minimumFractionDigits: 2 }
  )}`
);
console.log(
  `Total final: $${result.total.toLocaleString("es-AR", {
    minimumFractionDigits: 2,
  })}`
);

// Verificar que el total sea correcto
const expectedCostosAdicionales = 2470.0 + 661000.0; // 663,470.00
const expectedTotal = purchaseData.subtotalPesos + expectedCostosAdicionales; // 2,154,333.50

console.log("\n✅ Verificación:");
console.log(
  `Costos adicionales esperados: $${expectedCostosAdicionales.toLocaleString(
    "es-AR",
    { minimumFractionDigits: 2 }
  )}`
);
console.log(
  `Costos adicionales calculados: $${result.totalImportCosts.toLocaleString(
    "es-AR",
    { minimumFractionDigits: 2 }
  )}`
);
console.log(
  `¿Son iguales? ${
    Math.abs(result.totalImportCosts - expectedCostosAdicionales) < 0.01
      ? "✅ SÍ"
      : "❌ NO"
  }`
);
...
console.log(
  `Total esperado: $${expectedTotal.toLocaleString("es-AR", {
    minimumFractionDigits: 2,
  })}`
);
console.log(
  `Total calculado: $${result.total.toLocaleString("es-AR", {
    minimumFractionDigits: 2,
  })}`
);
console.log(
  `¿Son iguales? ${
    Math.abs(result.total - expectedTotal) < 0.01 ? "✅ SÍ" : "❌ NO"
  }`
);
...
if (Math.abs(result.totalImportCosts - expectedCostosAdicionales) < 0.01) {
  ...
  console.log(
    "✅ El resumen ahora muestra los valores reales, no los de la base de datos"
  );
} else {
