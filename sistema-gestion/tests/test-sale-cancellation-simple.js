...
async function testCancellation() {
  ...
  try {
    ...
    const afterSaleResponse1 = await fetch(`${BASE_URL}/api/products/${product1.id}`);
    const afterSaleResponse2 = await fetch(`${BASE_URL}/api/products/${product2.id}`);
    
    const afterSaleProduct1 = await afterSaleResponse1.json();
    const afterSaleProduct2 = await afterSaleResponse2.json();
    
    console.log(`📦 ${product1.name}: ${initialStock1} → ${afterSaleProduct1.data.stock} (diferencia: -${initialStock1 - afterSaleProduct1.data.stock})`);
    console.log(`📦 ${product2.name}: ${initialStock2} → ${afterSaleProduct2.data.stock} (diferencia: -${initialStock2 - afterSaleProduct2.data.stock})`);
    
    // 4. Cancelar la venta
    console.log('\n4. Cancelando la venta...');
    const cancelResponse = await fetch(`${BASE_URL}/api/sales/${saleResult.data.id}`, {
      method: 'DELETE'
    });
    ...
    const finalResponse1 = await fetch(`${BASE_URL}/api/products/${product1.id}`);
    const finalResponse2 = await fetch(`${BASE_URL}/api/products/${product2.id}`);
    
    const finalProduct1 = await finalResponse1.json();
    const finalProduct2 = await finalResponse2.json();
    
    console.log(`📦 ${product1.name}: ${afterSaleProduct1.data.stock} → ${finalProduct1.data.stock} (diferencia: +${finalProduct1.data.stock - afterSaleProduct1.data.stock})`);
    console.log(`📦 ${product2.name}: ${afterSaleProduct2.data.stock} → ${finalProduct2.data.stock} (diferencia: +${finalProduct2.data.stock - afterSaleProduct2.data.stock})`);
    ...
    console.log(`${test1Pass ? '✅' : '❌'} ${product1.name}: ${test1Pass ? 'CORRECTO' : 'ERROR'} (Inicial: ${initialStock1}, Final: ${finalProduct1.data.stock})`);
    console.log(`${test2Pass ? '✅' : '❌'} ${product2.name}: ${test2Pass ? 'CORRECTO' : 'ERROR'} (Inicial: ${initialStock2}, Final: ${finalProduct2.data.stock})`);
