...
async function createTestProducts() {
  ...
  for (const product of products) {
    try {
      ...
      console.log(
        `✅ Producto creado: ${result.data.name} (Stock: ${result.data.stock})`
      );
      createdProducts.push(result.data);
    } catch (error) {
      console.error(
        `❌ Error creando producto ${product.name}:`,
        error.message
      );
      throw error;
    }
...
async function getProductStock(productId) {
  try {
    ...
    console.error(
      `❌ Error obteniendo stock del producto ${productId}:`,
      error.message
    );
    throw error;
  }
...
async function checkStockMovements(products) {
  ...
  try {
    ...
    const cancellationMovements = movements.filter(
      (m) =>
    ...
    console.log(
      `📊 Movimientos de cancelación encontrados: ${cancellationMovements.length}`
    );
...
async function runTest() {
  console.log(
    "🧪 INICIANDO TEST DE CANCELACIÓN DE VENTAS CON DEVOLUCIÓN DE STOCK"
  );
  ...
  try {
    ...
    for (const product of products) {
      ...
      console.log(
        `📦 Stock inicial ${product.name}: ${initialStocks[product.id]}`
      );
    }
    ...
    for (const product of products) {
      ...
      console.log(
        `📦 Stock ${product.name}: ${stockAfterSimpleSale[product.id]}`
      );
    }
    ...
    for (const product of products) {
      stockAfterSimpleCancellation[product.id] = await getProductStock(
        product.id
      );
      console.log(
        `📦 Stock ${product.name}: ${stockAfterSimpleCancellation[product.id]}`
      );
    }
    ...
    for (const product of products) {
      ...
      console.log(
        `📦 Stock ${product.name}: ${stockAfterComboSale[product.id]}`
      );
    }
    ...
    for (const product of products) {
      ...
      if (expected === actual) {
        console.log(
          `✅ ${product.name}: Stock restaurado correctamente (${actual})`
        );
      } else {
        console.log(
          `❌ ${product.name}: ERROR - Esperado: ${expected}, Actual: ${actual}`
        );
    ...
    if (allTestsPassed) {
      console.log(
        "🎉 TODOS LOS TESTS PASARON - La cancelación devuelve el stock correctamente"
      );
    } else {
      console.log(
        "💥 ALGUNOS TESTS FALLARON - Revisar la lógica de cancelación"
      );
    }
  } catch (error) {
