...
export async function POST(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      console.log(`🔄 Intento ${attempt}/${maxAttempts} - Completando compra ${params.id}`);
      ...
      const result = await prisma.$transaction(async (tx) => {
        ...
        const totalImportCosts = freightCostARS + otherCostsARS + (purchase.customsCost || 0) + (purchase.taxCost || 0) + (purchase.insuranceCost || 0);
      ...
      console.log(`✅ Compra ${params.id} completada exitosamente en el intento ${attempt}`);
      return NextResponse.json({
      ...
      console.error(`❌ Error en intento ${attempt}/${maxAttempts}:`, {
        ...
        itemCount: await prisma.purchase.findUnique({ 
          where: { id: params.id }, 
  ...
  console.error(`💥 Error al completar compra ${params.id} después de ${maxAttempts} intentos:`, lastError);
  
  return NextResponse.json(
    { 
      error: `Error al completar la compra después de ${maxAttempts} intentos: ${lastError?.message || 'Error desconocido'}`,
      code: lastError?.code,
      attempt: maxAttempts
    },
    { status: 500 }
